(ns tombooth.friend-token.token-store-test
  (:require [clojure.test :refer :all]
            ...
            [tombooth.friend-token :as workflow]
            [tombooth.friend-token.token-store :as store]))

(deftest create-test
  (testing "Test creating a token"
    (let [tokens (atom {})
          token-store (store/->MemTokenStore (workflow/generate-key) 10 tokens)]
...
(deftest valid-fail-ttl-test
  (testing "Test getting metadata fails if outside ttl"
    (let [tokens (atom {})
          token-store (store/->MemTokenStore (workflow/generate-key) 1 tokens)
          token-hex (store/create token-store {:username "foo"})]
      (Thread/sleep 2000)
...
(deftest valid-extend-test
  (testing "Test extend works"
    (let [tokens (atom {})
          token-store (store/->MemTokenStore (workflow/generate-key) 2 tokens)
          token-hex (store/create token-store {:username "foo"})]
      (Thread/sleep 1000)
